...
constant data function, which allows a sheet to be produced without any variable data on it. To generate the
...
The coordinate system used depends on the object and how it is included in line data. Table 10summarizes
...
The Presentation Text (PTX) structured field is used to specify text data and the position, rotation, and fonts to
be used when presenting text data. The PTX structured field was previously known as Composed Text (CTX),
...
also place
Begin Presentation Text (BPT) and End Presentation Text (EPT) structured fields around each embedded
...
PTX is used to create a subsequent IPDS Write Text command. If a large number of PTX structured fields
...
PT2 that contains the Set Extended Text Color (SEC) control sequence for supporting spot colors and process
...
Presentation Text identifier (X'D3EE9B'). Following that is the X'00' flag byte and the two-byte sequence
...
Programming Tip
When deciding how to code Presentation Text structured fields, keep in mind that it is good programming
practice to build as long a PTX structured field as possible, to reduce overhead in the print server associated
with reading and processing many short records written by the application. Text control sequences should be
...
measure specified in the Presentation Text Descriptor (PTD) structured field. If the rule is to be drawn in the
...
To obtain the value to use when drawing this rule in the opposite direction, you calculate the two's complement
...
The third and fourth data bytes of the draw rule control sequence specify the thickness of the rule. To
...
These details come into play when drawing boxes with mitered corners. To make the box outline complete and
...
PTX structured fields must be bracketed by Begin Presentation Text (BPT) and End Presentation Text (EPT)
...
Programmers are free to include Begin Named Page Group (BNG), End Named Page Group (ENG), and Tag
