...
0 UBIN Tlength 3–254 Length of the triplet, including the length of
Tlength
M X'06'
1 CODE Tid X'01'–X'02', X'04',
X'10', X'18', X'1F',
X'20'–X'22', X'24'–
X'26', X'2D', X'36',
X'43', X'45', X'4B'–
...
• The most recent occurrence of a X'01' triplet on a structured field establishes the code page and character
set used to interpret all subsequent parameters within that structured field with a CHAR data type.
• If the structured field syntax allows parameters with a CHAR data type to be positioned before the allowed
...
• If X'01' triplets appear on a Begin structured field, the last X'01' triplet specified establishes the code page
...
Application Notes:
1. Most MO:DCA character strings are carried in Fully Qualified Name (FQN) triplets. This triplet limits the
...
• An encoded graphic character string that, when qualified by the associated CGCSGID, specifies a reference
name
• An identifier used by a data object to reference a resource
• A Uniform Resource Locator (URL), as defined in RFC 1738, Internet Engineering T ask Force (IETF),
December, 1994
...
Reference
M X'06'
3 CODE FQNFmt X'00', X'10', X'20' Specifies the GID format:
...
• specify a Link LK Color Conversion CMR that is mapped to the CMR
in the container, or
• specify a device-specific HT or TTC CMR replacement for a generic
HT or TTC CMR
...
• If the BRS envelopes a TTF/OTF container, the FQN type X'7E' triplet
specifies a linked TTF/OTF for the font in the container.
• If the BRS envelopes a TTC container, the FQN type X'7E' triplet
...
• Each component identifier, except for the first two which are treated as a special case, is represented as a
...
B'0 000 0011' = X'03'.
• The first two component identifiers, represented by x and y in the OID (x.y.z.....), are combined into a single
...
• For fixed-pitch, uniform character increment fonts, including Proportional Spacing Machine (PSM) fonts, the
...
1. For IBM Core Interchange Courier fonts, and for IBM Expanded Core fonts with FGID values less than
750 and with FGID values between 3,840 and 4,095 inclusive (fixed pitch, uniform character increment,
...
field is ignored.
Notes:
1. For presentation objects, a presentation space size is required for a
...
page or overlay.
2. The Scale to fit mapping option has value X'10' in IPDS and in BCOCA.
...
Notes:
1. These bits must be set to B'11' when the triplet appears on a Begin Object
Container (BOC) structured field.
2. When bits 0–1 are set to B'11', bits 4–5 must also be set to B'11'.
3. It is not advisable to set the bits to B'11' when the triplet appears on a structured
...
Notes:
1. When bits 2–3 are set to B'11', bits 0–1 must be set to B'11', and bits 4–5 must be
set to B'11'.
2. It is not advisable to set the bits to B'01' when the triplet appears on a structured
field that references the object such as an Include Object (IOB), since the
...
Notes:
1. When bits 4–5 are set to B'11', bits 0–1 must also be set to B'11'. Conversely,
...
All others Reserved
Architecture Notes:
1. ISid X'0C00' is used in MO:DCA-L data streams with ISType X'03' to indicate a Resource (
...
2. For ISType X'01', the ISid two-byte code is treated architecturally as two fields of one byte
...
Architecture Notes:
• The local IDs used with resource type X'40' are specified with a X'E8nn' + X'E9nn'
...
• The MMO, MDD, MCC, MFC with MFCScpe = X'04' (medium map level MFC, each sheet),
...
follows:
• Default page placement: The next partition is the next sequential partition
...
• Explicit page placement: The next partition is defined by the repeating
...
• Default page placement: If currently placing pages on the front sheet side,
...
• Explicit page placement: The next front-side partition is defined by the
...
• Default page placement: If currently placing pages on the back sheet side,
...
ColSizeN
- 1), where N=1,2,3,4. This is a presentation-system-dependent color space.
...
Architecture Notes:
1. The color that is rendered when a highlight color is specified is
...
3. On printing devices, the color of medium is normally white, in which case
...
• Color Management Resource (CMR) exceptions. This does not include unsupported CMR tag exceptions,
...
ignored.
• Device Appearance exceptions. A Device Appearance exception is detected when a requested appearance
...
• If the Color Fidelity triplet is not specified but is supported by the printer, the printer is instructed to
...
• If this operation is specified immediately after a finishing operation that
...
• If the T ext Fidelity triplet is specified and is supported by the printer, the triplet is sent to the printer and
...
• If the T ext Fidelity triplet is specified and is not supported by the printer, the triplet is processed by the
...
1. Attempt to find an available media source containing the media type that matches the specified
...
• If the Finishing Fidelity triplet is specified and is supported by the printer, the triplet is sent to the printer
and processed by both server and printer.
• If the Finishing Fidelity triplet is specified and is not supported by the printer, the triplet is processed by
...
Bits Description
0 MICR print. Defines whether the font is to be used for Magnetic Ink Character
...
0 degrees; an upside down character has a character rotation of 180 degrees. A character
...
Architecture Notes:
1. The character rotation parameter is used in PTOCA text objects along with the current
inline and baseline directions to determine the character orientation with respect to the
page (X
p, Yp) coordinate system.
2. The character-rotation parameter applies only to characters used in PTOCA text objects or
...
• The TrueType/OpenType font technology uses an internal cmap table for this purpose; most
...
• The definition of script codes can be found at:
www.unicode.org/reports/tr24
• The definition of region codes can be found at:
...
• If a X'8C' triplet appears on an MDR structured field that references a data-object font, its scope is the text
...
3166 Alpha-3 code) defined in ISO 3166-1.
All
others
Reserved
...
Architecture Notes:
1. The rendering intent for bar code (BCOCA) objects and for IM-image objects cannot be specified with the
...
processed:
• If the CMR T ag Fidelity triplet is specified and is supported by the printer, the triplet is sent to the
...
• If the CMR T ag Fidelity triplet is specified and is not supported by the printer, the triplet is processed by
...
• When the mapping option is scale-to-fill, the pixel counts are sufficient since the intent is to scale the
