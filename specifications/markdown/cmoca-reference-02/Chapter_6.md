...
1.  If the appropriate **AToBxTag** (or **BToAxTag**) for that rendering intent exists, it is used. (**AToB0Tag** is used for perceptual, **AToB1Tag** is used for media-relative colorimetric, **AToB2Tag** is used for saturation.) [CMOCA-6-080]
...
For **audit** CMRs, only the **AToBxTags** are considered, not the **BToAxTags**. If no **AToBxTags** exist:
...
For **instruction** CMRs, only the **BToAxTags** are considered, not the **AToBxTags**. If no **BToAxTags** exist:
...
A CMR is **passthrough** if the **CMRVersion** in the CMR Header is “pasthru”. [CMOCA-6-188]
Only **Color Conversions** may be **passthrough** so, if the CMR type is anything else, an exception occurs. [CMOCA-6-189]
The mode of a **passthrough CC** CMR must be **audit**. [CMOCA-6-190]
The CMR is ignored if the mode is specified as **instruction**. [CMOCA-6-191]
If the mode is **link**, an exception is generated since that is not valid for any **Color Conversion** CMRs. [CMOCA-6-192]
