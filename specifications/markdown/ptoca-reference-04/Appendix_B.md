...
semantic definitions for the supported PTOCA subset.
...
1. If a receiver cannot provide the color specified by FRGCOLOR, it may substitute values from Table 13 on
 defined in the STC control sequence.
...
graphic characters, is carried in the IPDS Write Text (WT) command. The subsets of PTOCA that are used by
...
not, the IPDS exception ID overrides the PTOCA exception condition code. The IPDS Architecture also defines
