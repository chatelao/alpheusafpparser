...
objects. There shall be no references to this dictionary anywhere in the document; however, the firstpage cross-reference table (part 3) shall contain a normal entry for it.
...
F.3.5, "Document catalog dictionary and document-level objects (Part 4)"). Additionally, this crossreference table shall contain entries for the linearization parameter dictionary (at the beginning) and
...
section shall be followed by the entries in "Table F.12 — Embedded file stream hint table, perembedded file stream group entries". Each of the items in "Table F.12 — Embedded file stream hint
...
header section"). That is, the order of items in "Table F.12 — Embedded file stream hint table, perembedded file stream group entries" shall be item 1 for the first group, item 1 for the second group,
