...
page pageNum A specified page number of the document. The argument shall be a positive
...
nameddest name A specified named destination (see 12.3.2.4, "Named destinations"). The
...
ISO 32000-2:2020
978 © ISO 2020 – All rights reserved
Parameter Arguments Description
structelem structID The structID is a byte string with URI encoding that matches the ID key
...
ef name The name argument shall be a byte string used to match a file specification
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 979
Parameter Arguments Description
view keyword,position Open the document with the specified destination set as the view. The
...
values.
viewrect left,top,width,height Open the document with the specified window view rectangle. The left
...
highlight left,right,top,bottom Open the document with the specified rectangle highlighted. Each
...
search wordList Open the document and search for one or more words, selecting the first
...
fdf URI Open the document and then import the data from the specified FDF or
XFDF file. The URI shall be either a relative or absolute URI to an FDF or
XFDF file.
The fdf parameter should be specified as the last parameter to a gi ven
URI.
...
ISO 32000-2:2020
980 © ISO 2020 – All rights reserved
