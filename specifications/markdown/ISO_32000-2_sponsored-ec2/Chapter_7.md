...
NOTE 2 In this clause, the usage of the term character is entirely independent of any logical meaning that
...
have zero entries.
NOTE 1 The statement requiring dictionary keys to always be direct objects was added in this document
(2020).
...
exactly 200 bytes of image data. If the stream use s a filter, there needs to be enough bytes of encoded data
...
Key Type Value
Length integer (Required) The number of bytes from the beginning of the line following the
...
for DecodeParms.
DL integer (Optional; PDF 1.5) A non-negative integer representing the number of bytes
...
Input sequence Output code Code added to
table
...
process.
NOTE Most of these parameters are related to techniques that reduce the size of compressed sampled
...
Key Type Value
Predictor integer A code that selects the predictor algorithm, if any. If the value of
...
Key Type Value
EarlyChange integer (LZWDecode only) An indication of when to increase the code
...
Average Predicts the average of the sample to the left and the
sample above
Paeth A nonlinear function of the sample above, the sample to
...
EndOfLine boolean A flag indicating whether end-of-line bit patterns shall be present in
...
EncodedByteAlign boolean A flag indicating whether the filter shall expect extra 0 bits before
...
EndOfBlock boolean A flag indicating whether the filter shall expect the encoded data to
...
BlackIs1 boolean A flag indicating whether bits with a value of 1 shall be interpreted
...
DamagedRowsBeforeError integer The number of damaged rows of data that shall be tolerated before
...
Key Type Value
ColorTransform integer (Optional) A code specifying the transformation that shall be
...
Key Type Value
Type name (Optional) If present, shall be CryptFilterDecodeParms for a
...
Key Type Value
Name name (Optional) The name of the crypt filter that shall be used to decrypt
...
subsection.
NOTE 4 The number of entries in a subsection for a file that has never been incrementally updated can
never be zero.
...
Prev integer (Optional, present only if the file has more than one cross-reference
...
Key Type Value
Root dictionary (Required; shall be an indirect reference) The catalog dictionary for the
PDF file (see 7.7.2, "Document catalog dictionary").
Encrypt dictionary (Required if document is encrypted; PDF 1.1) The PDF file’s encryption
...
EXAMPLE This example shows a trailer for a PDF file that has n ever been updated (as indicated by the absence of a
...
First integer (Required) The byte offset in the decoded stream of the first compresse d
object.
Extends stream (Optional) A reference to another object stream, of which the current
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 65
stream
  00 0000 FFFF
...
Key Type Value
Prev integer (Required, if any cross reference streams are already present in the file )
...
0 1 The type of this entry, which shall be 0. Type 0 entries
...
1 1 The type of this entry, which shall be 1. Type 1 entries
...
2 1 The type of this entry, which shall be 2. Type 2 entries
...
Key Type Value
XRefStm integer (Optional) The byte offset in the decoded stream from the beginning of the
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 69
...
EXAMPLE In this example, an  ASCIIHexDecode filter is specif ied to make the format and contents of the cross -
...
no structure tree.
 Another choice for a PDF reader, is to conti nue the search by following the  XRefStm pointer to the cross -
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 71
<< /Size 100
...
ISO 32000-2:2020(E)
72 © ISO 2020 – All rights reserved
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 73
Key Type Value
...
ISO 32000-2:2020(E)
74 © ISO 2020 – All rights reserved
Key Type Value
...
this value when encrypting embedded files, except for embedded fi le streams that
...
determine the original message length w hen decrypting. Note that the pad is present when M is evenly
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 75
...
ISO 32000-2:2020(E)
76 © ISO 2020 – All rights reserved
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 77
...
ISO 32000-2:2020(E)
78 © ISO 2020 – All rights reserved
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 79
Key Type Value
Perms byte
string
...
a faithful digital copy of the PDF content could be generated, based on an implementat ion-
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 85
...
7.6.4.4.9 Algorithm 10: Computing the encryption dictionary’s Perms (permissions)
...
a multiple of 8 in the range of 40 to 256. The standard security handle r
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 93
Key Type Value
...
ISO 32000-2:2020(E)
94 © ISO 2020 – All rights reserved
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 95
...
EXAMPLE This example  shows details of an unencrypted wrapper document containing an encrypted p ayload
document.
%PDF-2.0
15 0 obj
...
ISO 32000-2:2020(E)
96 © ISO 2020 – All rights reserved
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 97
...
annotations ( 12.5, " Annotations") associa ted with it. The individual page objects are tied together in a
...
ISO 32000-2:2020(E)
98 © ISO 2020 – All rights reserved
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 99
Key Type Value
...
Extensions dictionary (Optional; ISO 32000-1) An extensions dictionary containing
...
Pages dictionary (Required; shall be an indirect reference) The page tree node that
shall be the root of the document’s page tree (see 7.7.3, "Page
tree").
PageLabels number tree (Optional; PDF 1.3) A number tree (see 7.9.7, "Number trees")
...
Names dictionary (Optional; PDF 1.2) The document’s name dictionary (see 7.7.4,
"Name dictionary").
...
Dests dictionary (Optional; PDF 1.1; shall be an indirect reference) A dictionary of
names and corresponding destinations (see 12.3.2.4, "Named
destinations").
ViewerPreferences dictionary (Optional; PDF 1.2) A viewer preferences dictionary (see 12.2,
...
ISO 32000-2:2020(E)
100 © ISO 2020 – All rights reserved
Key Type Value
PageLayout name (Optional) A name object specifying the page layout shall be used
...
PageMode name (Optional) A name object specifying how the document shall be
...
Outlines dictionary (Optional; shall be an indirect reference) The outline dictionary that
shall be the root of the document’s outline hierarchy (see 12.3.3,
"Document outline").
Threads array (Optional; PDF 1.1; shall be an indirect reference) An array of thread
...
URI dictionary (Optional; PDF 1.1) A URI dictionary containing document-level
information for URI (uniform resource identifier) actions (see
12.6.4.8, "URI actions").
AcroForm dictionary (Optional; PDF 1.2) The document’s interactive form (AcroForm)
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 101
Key Type Value
Metadata stream (Optional; PDF 1.4; shall be an indirect reference) A metadata stream
that shall contain metadata for the document (see 14.3.2, "Metadata
streams").
StructTreeRoot dictionary (Optional; PDF 1.3) The document’s structure tree root dictionary
(see 14.7.2, "Structure hierarchy").
MarkInfo dictionary (Optional; PDF 1.4) A mark information dictionary that shall contain
information about the document’s usage of tagged PDF conventions
(see 14.7, "Logical structure").
Lang text string (Optional; PDF 1.4) A language identifier that shall specify the
...
SpiderInfo dictionary (Optional; PDF 1.3) A Web Capture information dictionary that shall
contain state information used by any Web Capture extension (see
14.10.2, "Web capture information dictionary").
OutputIntents array (Optional; PDF 1.4) An array of output intent dictionaries that shall
specify the colour characteristics of output devices on which the
document might be rendered (see 14.11.5, "Output intents").
PieceInfo dictionary (Optional; PDF 1.4) A page-piece dictionary associated with the
document (see 14.5, "Page-piece dictionaries").
OCProperties dictionary (Optional; PDF 1.5; required if a document contains optional content)
The document’s optional content properties dictionary (see 8.11.4,
"Configuring optional content").
Perms dictionary (Optional; PDF 1.5) A permissions dictionary that shall specify user
access permissions for the document. 12.8.6, "Permissions",
describes this dictionary and how it shall be used.
Legal dictionary (Optional; PDF 1.5) A dictionary that shall contain attestations
...
Requirements array (Optional; PDF 1.7) An array of requirement dictionaries that shall
represent requirements for the document. Subclause 12.11,
"Document requirements", describes this dictionary and how it
shall be used.
Collection dictionary (Optional; PDF 1.7) A collection dictionary that an interactive PDF
...
ISO 32000-2:2020(E)
102 © ISO 2020 – All rights reserved
Key Type Value
NeedsRendering boolean (Optional; deprecated in PDF 2.0) A flag used to expedite the display
...
Default value: false.
DSS dictionary (Optional; PDF 2.0) A DSS dictionary containing document-wide
...
DPartRoot dictionary (Optional; PDF 2.0) A DPartRoot dictionary used to describe the
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 103
Table 30 — Required entries in a page tree node
Key Type Value
Type name (Required) The type of PDF object that this dictionary describes; shall be  Pages
for a page tree node.
Parent dictionary (Required except in root node; not permitted in the root node; shall be an
...
ISO 32000-2:2020(E)
104 © ISO 2020 – All rights reserved
...
Parent dictionary (Required; shall be an indirect reference) The page tree node that is the
...
Resources dictionary (Required; inheritable) A dictionary containing any resources required
...
MediaBox rectangle (Required; inheritable) A rectangle (see 7.9.5, "Rectangles"), expressed
...
CropBox rectangle (Optional; Inheritable) A rectangle, expressed in default user space
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 105
Key Type Value
BleedBox rectangle (Optional; PDF 1.3) A rectangle, expressed in default user space units,
...
TrimBox rectangle (Optional; PDF 1.3) A rectangle, expressed in default user space units,
...
CropBox.
ArtBox rectangle (Optional; PDF 1.3) A rectangle, expressed in default user space units,
...
BoxColorInfo dictionary (Optional; PDF 1.4) A box colour information dictionary that shall
...
Rotate integer (Optional; inheritable) The number of degrees by which the page shall
be rotated clockwise when displayed or printed. The value shall be a
multiple of 90. Default value: 0.
Group dictionary (Optional; PDF 1.4) A group attributes dictionary that shall specify the
...
XObjects").
Thumb stream (Optional) A stream object that shall define the page’s thumbnail image
...
ISO 32000-2:2020(E)
106 © ISO 2020 – All rights reserved
Key Type Value
...
Annots array (Optional) An array of annotation dictionaries that shall contain
indirect references to all annotations associated with the page (see
12.5, "Annotations").
AA dictionary (Optional; PDF 1.2) An additional-actions dictionary that shall define
...
inheritable.
Metadata stream (Optional; PDF 1.4) A metadata stream that shall contain metadata for
the page (see 14.3.2, "Metadata streams").
PieceInfo dictionary (Optional; PDF 1.3) A page-piece dictionary associated with the page
(see 14.5, "Page-piece dictionaries").
StructParents integer (Required if the page contains structural content items; PDF 1.3 ) The
...
PZ number (Optional; PDF 1.3) The page’s preferred zoom (magnification) factor:
...
SeparationInfo dictionary (Optional; PDF 1.3) A separation dictionary that shall contain
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 107
Key Type Value
Tabs name (Optional; PDF 1.5) A name specifying the tab order that shall be used
...
TemplateInstantiated name (Required if this page was created from a named page object; PDF 1.5 )
The name of the originating page object (see 12.7.7, "Named pages").
PresSteps dictionary (Optional; PDF 1.5) A navigation node dictionary that shall represent
the first node on the page (see 12.4.4.2, "Sub-page navigation").
UserUnit number (Optional; PDF 1.6) A positive number that shall give the size of default
...
VP array (Optional; PDF 1.6) An array of viewport dictionaries (see "Table 265
...
OutputIntents array (Optional; PDF 2.0) An array of output intent dictionaries that shall
specify the colour characteristics of output devices on which this page
might be rendered (see 14.11.5, "Output intents").
DPart dictionary (Required, if this page is within the range of a DPart, not permitted
...
ISO 32000-2:2020(E)
108 © ISO 2020 – All rights reserved
...
node of the page tree. If necessary, an individual page ob ject can override this inherited value with a
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 109
...
ISO 32000-2:2020(E)
110 © ISO 2020 – All rights reserved
Key Type Value
EmbeddedFiles name
tree
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 111
...
ISO 32000-2:2020(E)
112 © ISO 2020 – All rights reserved
Table 33 — Compatibility operators
Operands Operator Description
— BX (PDF 1.1) Begin a compatibility section. Unrecognised operators
(along with their operands) shall be ignored without error until the
balancing EX operator is encountered.
— EX (PDF 1.1) End a compatibility section begun by a balancing BX
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 113
...
Key Type Value
ExtGState dictionary (Optional) A dictionary that maps resource names to graphics
state parameter dictionaries (see 8.4.5, "Graphics state parameter
dictionaries").
ColorSpace dictionary (Optional) A dictionary that maps each resource name to either the
name of a device-dependent colour space or an array describing a
colour space (see 8.6, "Colour spaces").
Pattern dictionary (Optional) A dictionary that maps resource names to pattern
objects (see 8.7, "Patterns").
Shading dictionary (Optional; PDF 1.3) A dictionary that maps resource names to
shading dictionaries (see 8.7.4.3, "Shading dictionaries").
XObject dictionary (Optional) A dictionary that maps resource names to external
objects (see 8.8, "External objects").
Font dictionary (Optional) A dictionary that maps resource names to font
dictionaries (see 9, "Text").
ProcSet array (Optional; deprecated in PDF 2.0) An array of predefined procedure
set names (see 14.2, "Procedure sets").
Properties dictionary (Optional; PDF 1.2) A dictionary that maps resource names to
...
ISO 32000-2:2020(E)
114 © ISO 2020 – All rights reserved
...
byte string A series of bytes that shall represent
...
file specification File specification (string or
dictionary)
7.11
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 115
Type Description Subclause
...
ISO 32000-2:2020(E)
116 © ISO 2020 – All rights reserved
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 117
...
ISO 32000-2:2020(E)
118 © ISO 2020 – All rights reserved
7.9.3 Text streams
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 119
...
ISO 32000-2:2020(E)
120 © ISO 2020 – All rights reserved
...
EXAMPLE 1 The following is an abbreviated outline, showing ob ject numbers and nodes, of a name tree that maps the
...
ISO 32000-2:2020(E)
122 © ISO 2020 – All rights reserved
5 0 obj
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 123
Key Type Value
Limits array (Shall be present in Intermediate and leaf nodes only) Shall be an array of two
...
ISO 32000-2:2020(E)
124 © ISO 2020 – All rights reserved
...
Domain array (Required) An array of 2 × m numbers, where m shall be the
...
value.
Range array (Required for Type 0 and Type 4 functions, optional otherwise; see
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 125
...
Key Type Value
Size array (Required) An array of m positive integers that shall specify the number of
...
ISO 32000-2:2020(E)
126 © ISO 2020 – All rights reserved
Key Type Value
BitsPerSample integer (Required) The number of bits that shall represent each sample. (If the
...
Order integer (Optional) The order of interpolation between samples. Valid values shall be
...
-1) 0 (Size1 - 1)…].
Decode array (Optional) An array of 2 × n numbers specifying the linear mapping of sample
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 127
EXAMPLE 1 For a function f(a, b, c), where a, b, and c vary from 0 to 9 in steps of 1, the sample value s would appear in
...
C1 array (Optional) An array of n numbers that shall define the function result when
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 129
...
Bounds array (Required) An array of 𝑘 −  1 numbers that, in combination with Domain, shall define
...
Encode array (Required) An array of 2 ×  𝑘 numbers that, taken in pairs, shall map each subset of
...
ISO 32000-2:2020(E)
130 © ISO 2020 – All rights reserved
(𝐵𝑜𝑢𝑛𝑑𝑠0, 𝐵𝑜𝑢𝑛𝑑𝑠1); [𝐵𝑜𝑢𝑛𝑑𝑠1, 𝐷𝑜𝑚𝑎𝑖𝑛1].
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 131
...
ISO 32000-2:2020(E)
132 © ISO 2020 – All rights reserved
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 133
...
ISO 32000-2:2020(E)
134 © ISO 2020 – All rights reserved
...
ISO 32000-2:2020(E)
136 © ISO 2020 – All rights reserved
Key Type Value
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 137
Key Type Value
...
ISO 32000-2:2020(E)
138 © ISO 2020 – All rights reserved
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 139
Key Type Value
...
ISO 32000-2:2020(E)
140 © ISO 2020 – All rights reserved
   (Sunset.M) 33 0 R
   (Sunset.Y) 34 0 R
   (Sunset.K) 35 0 R
   ]
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 141
Key Type Value
...
EXAMPLE If the c orresponding collection field has a Subtype entry of S, then the
...
P text string (Optional) A prefix string that shall be concatenated with the text string presented to the
...
ISO 32000-2:2020(E)
142 © ISO 2020 – All rights reserved
objects used).
...
Key Type Value
Type name (Optional; , shall be a direct objectif present) The type of PDF object that this
...
Key Type Value
Type name (Optional; shall be a direct object if present) The type of PDF object that this
dictionary describes; if present, shall be DeveloperExtensions.
BaseVersion name (Required; shall be a direct object) The name of the PDF version to which
...
catalog dictionary").
ExtensionLevel integer (Required; shall be a direct object) An integer defined by the developer to
...
URL string (Optional; PDF 2.0; shall be a direct object if present) A URL that refers to the
...
ISO 32000-2:2020(E)
© ISO 2020 – All rights reserved 143
...
ISO 32000-2:2020(E)
144 © ISO 2020 – All rights reserved
