...
public class PdfHandler implements StructuredFieldHandler {
          ...
          if (irg instanceof MCF_MapCodedFont_Format2.MCF_RepeatingGroup rg && rg.getTriplets() != null) {
            ...
            for (Triplet t : rg.getTriplets()) {
              if (t instanceof Triplet.ResourceLocalIdentifier rli
                ...
                if (fds.fontHeight > 0) {
                  // Font Height in Triplet X'1F' is in 1/1440 inch units (1/20 points)
            ...
            for (Triplet t : rg.getTriplets()) {
              if (t instanceof Triplet.ResourceLocalIdentifier rli
                ...
                if (dofd.specifiedVerticalFontSize > 0) {
                  // Font Size in Triplet X'43' is in 1/1440 inch units (1/20 points)
  ...
  private void drawWindowOverlay(PdfPage page) {
    ...
    // Outer window (Pink) - 119x72mm (19mm wider, 27mm taller than mauve box), aligned concentric around center2
    float pinkW = mauveW + (float) (19.0 * mmToPoints);
    float pinkH = mauveH + (float) (27.0 * mmToPoints);
    float pinkX = center2X - pinkW / 2.0f;
    float pinkY = center2Y - pinkH / 2.0f;

    // Inner window (Purple) - 81x18mm (19mm narrower, 27mm shorter than mauve box), aligned concentric inside center2
    float purpleW = mauveW - (float) (19.0 * mmToPoints);
    float purpleH = mauveH - (float) (27.0 * mmToPoints);
