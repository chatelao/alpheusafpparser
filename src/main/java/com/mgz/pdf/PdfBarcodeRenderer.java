...
public class PdfBarcodeRenderer {
  ...
  static {
    ...
    // Codabar pattern: 4 bars, 3 spaces. 'w' for wide, 'n' for narrow.
  ...
  private static float renderCodabar(String content, int x, int y, PdfBarcodeState state, PdfCanvas canvas) {
    ...
    if (!content.isEmpty() && !"ABCD".contains(String.valueOf(content.charAt(0)))) {
      content = "A" + content;
    }
    if (!content.isEmpty() && !"ABCD".contains(String.valueOf(content.charAt(content.length() - 1)))) {
      content = content + "A";
    }
    ...
    for (int i = 0; i < content.length(); i++) {
      ...
      // Inter-character gap (usually narrow space)
