...
public class AfpJacksonXmlWriter implements StructuredFieldHandler {
  ...
  private void writeFieldDirectly(StructuredField sf) throws Exception {
    ...
    if (sf instanceof NOP_NoOperation nop) {
      ...
      resetPtocaState();
      writeShsDirectly(shs, level);
    } else if (sf instanceof com.mgz.afp.ipds.END_End end) {
    ...
    } else if (sf instanceof com.mgz.afp.ipds.LFCSC_LoadFontCharacterSetControl lfcsc) {
      writeLfcscDirectly(lfcsc, level);
    } else if (sf instanceof com.mgz.afp.ipds.LCPC_LoadCodePageControl lcpc) {
      writeLcpcDirectly(lcpc, level);
    } else if (sf instanceof com.mgz.afp.ipds.LF_LoadFont lf) {
      writeLfIpdsDirectly(lf, level);
    } else if (sf instanceof com.mgz.afp.ipds.LCP_LoadCodePage lcp) {
      writeLcpDirectly(lcp, level);
    } else if (sf instanceof com.mgz.afp.ipds.LSS_LoadSymbolSet lss) {
      writeLssDirectly(lss, level);
    ...
    } else if (sf instanceof com.mgz.afp.ipds.LE_LoadEquivalence le) {
      writeLeDirectly(le, level);
    } else if (sf instanceof com.mgz.afp.ipds.WT_WriteText wt) {
      writeWtDirectly(wt, level);
  ...
  private void writeEpDirectly(com.mgz.afp.ipds.EP_EndPage ep, int level) throws Exception {
    ...
    int childLevel = level + 1;
    writeElement(baseXsw, childLevel, "flagByte", (int) ep.getFlagByte());
    if (ep.getCorrelationId() != null) {
      writeElement(baseXsw, childLevel, "correlationId", ep.getCorrelationId());
    }
    if (ep.getText() != null) {
  ...
  private void writeShsDirectly(com.mgz.afp.ipds.SHS_SetHomeState shs, int level) throws Exception {
    MnemonicPerformanceMonitor.startWriteWithMnemonic("SHS");
    baseXsw.writeStartElement("SHS_SetHomeState");
    if (currentPageNumber > 0) {
      baseXsw.writeIntAttribute(null, null, "page", currentPageNumber);
    }
    int childLevel = level + 1;
    writeElement(baseXsw, childLevel, "flagByte", (int) shs.getFlagByte());
    if (shs.getCorrelationId() != null) {
      writeElement(baseXsw, childLevel, "correlationId", shs.getCorrelationId());
    }
    if (shs.getText() != null) {
      writeElement(baseXsw, childLevel, "text", shs.getText());
    }
    writeIndent(baseXsw, level);
    baseXsw.writeEndElement();
    MnemonicPerformanceMonitor.endWrite();
  ...
  private void writeEndDirectly(com.mgz.afp.ipds.END_End end, int level) throws Exception {
    MnemonicPerformanceMonitor.startWriteWithMnemonic("END");
    baseXsw.writeStartElement("END_End");
    if (currentPageNumber > 0) {
      baseXsw.writeIntAttribute(null, null, "page", currentPageNumber);
    }
    int childLevel = level + 1;
    writeElement(baseXsw, childLevel, "flagByte", (int) end.getFlagByte());
    if (end.getCorrelationId() != null) {
      writeElement(baseXsw, childLevel, "correlationId", end.getCorrelationId());
    }
    if (end.getText() != null) {
      writeElement(baseXsw, childLevel, "text", end.getText());
    }
    writeIndent(baseXsw, level);
    baseXsw.writeEndElement();
    MnemonicPerformanceMonitor.endWrite();
  ...
  private void writeStmDirectly(com.mgz.afp.ipds.STM_SenseTypeAndModel stm, int level) throws Exception {
    MnemonicPerformanceMonitor.startWriteWithMnemonic("STM");
    baseXsw.writeStartElement("STM_SenseTypeAndModel");
    if (currentPageNumber > 0) {
      baseXsw.writeIntAttribute(null, null, "page", currentPageNumber);
    }
    int childLevel = level + 1;
    writeElement(baseXsw, childLevel, "flagByte", (int) stm.getFlagByte());
    if (stm.getCorrelationId() != null) {
      writeElement(baseXsw, childLevel, "correlationId", stm.getCorrelationId());
    }
    if (stm.getText() != null) {
      writeElement(baseXsw, childLevel, "text", stm.getText());
    }
    writeIndent(baseXsw, level);
    baseXsw.writeEndElement();
    MnemonicPerformanceMonitor.endWrite();
  ...
  private void writeLfcscDirectly(com.mgz.afp.ipds.LFCSC_LoadFontCharacterSetControl lfcsc, int level)
      throws Exception {
    MnemonicPerformanceMonitor.startWriteWithMnemonic("LFCSC");
    baseXsw.writeStartElement("LFCSC_LoadFontCharacterSetControl");
    if (currentPageNumber > 0) {
      baseXsw.writeIntAttribute(null, null, "page", currentPageNumber);
    }
    int childLevel = level + 1;
    writeElement(baseXsw, childLevel, "flagByte", (int) lfcsc.getFlagByte() & 0xFF);
    if (lfcsc.getCorrelationId() != null) {
      writeElement(baseXsw, childLevel, "correlationId", lfcsc.getCorrelationId());
    }
    writeElement(baseXsw, childLevel, "haid", lfcsc.getHaid());
    writeElement(baseXsw, childLevel, "patternTechnology", (int) lfcsc.getPatternTechnology() & 0xFF);
    writeElement(baseXsw, childLevel, "intendedUseFlags", (int) lfcsc.getIntendedUseFlags() & 0xFF);
    writeElement(baseXsw, childLevel, "loadFontCount", lfcsc.getLoadFontCount());
    writeElement(baseXsw, childLevel, "mapSize", lfcsc.getMapSize());
    writeElement(baseXsw, childLevel, "characterIdCount", lfcsc.getCharacterIdCount());
    if (lfcsc.getGcsgid() != null) {
      writeElement(baseXsw, childLevel, "gcsgid", lfcsc.getGcsgid());
    }
    if (lfcsc.getFgid() != null) {
      writeElement(baseXsw, childLevel, "fgid", lfcsc.getFgid());
    }
    writeIndent(baseXsw, level);
    baseXsw.writeEndElement();
    MnemonicPerformanceMonitor.endWrite();
  ...
  private void writeLcpcDirectly(com.mgz.afp.ipds.LCPC_LoadCodePageControl lcpc, int level)
      throws Exception {
    MnemonicPerformanceMonitor.startWriteWithMnemonic("LCPC");
    baseXsw.writeStartElement("LCPC_LoadCodePageControl");
    if (currentPageNumber > 0) {
      baseXsw.writeIntAttribute(null, null, "page", currentPageNumber);
    }
    int childLevel = level + 1;
    writeElement(baseXsw, childLevel, "flagByte", (int) lcpc.getFlagByte() & 0xFF);
    if (lcpc.getCorrelationId() != null) {
      writeElement(baseXsw, childLevel, "correlationId", lcpc.getCorrelationId());
    }
    writeElement(baseXsw, childLevel, "haid", lcpc.getHaid());
    writeElement(baseXsw, childLevel, "encodingScheme", lcpc.getEncodingScheme());
    writeElement(baseXsw, childLevel, "byteCount", lcpc.getByteCount());
    writeElement(baseXsw, childLevel, "extensionFlags", (int) lcpc.getExtensionFlags() & 0xFF);
    if (lcpc.getVariableSpaceCodePoint() != null) {
      writeElement(baseXsw, childLevel, "variableSpaceCodePoint",
          com.mgz.util.UtilCharacterEncoding.bytesToHexString(lcpc.getVariableSpaceCodePoint()));
    }
    if (lcpc.getGcsgid() != null) {
      writeElement(baseXsw, childLevel, "gcsgid", lcpc.getGcsgid());
    }
    if (lcpc.getCpgid() != null) {
      writeElement(baseXsw, childLevel, "cpgid", lcpc.getCpgid());
    }
    if (lcpc.getDefaultGcgid() != null) {
      writeElement(baseXsw, childLevel, "defaultGcgid", lcpc.getDefaultGcgid());
    }
    if (lcpc.getDefaultProcessingFlags() != null) {
      writeElement(baseXsw, childLevel, "defaultProcessingFlags",
          (int) lcpc.getDefaultProcessingFlags() & 0xFF);
    }
    if (lcpc.getDefaultUnicodeScalarValue() != null) {
      writeElement(baseXsw, childLevel, "defaultUnicodeScalarValue",
          lcpc.getDefaultUnicodeScalarValue());
    }
    writeIndent(baseXsw, level);
    baseXsw.writeEndElement();
    MnemonicPerformanceMonitor.endWrite();
  ...
  private void writeLfIpdsDirectly(com.mgz.afp.ipds.LF_LoadFont lf, int level) throws Exception {
    MnemonicPerformanceMonitor.startWriteWithMnemonic("LF");
    baseXsw.writeStartElement("LF_LoadFont");
    if (currentPageNumber > 0) {
      baseXsw.writeIntAttribute(null, null, "page", currentPageNumber);
    }
    int childLevel = level + 1;
    writeElement(baseXsw, childLevel, "flagByte", (int) lf.getFlagByte() & 0xFF);
    if (lf.getCorrelationId() != null) {
      writeElement(baseXsw, childLevel, "correlationId", lf.getCorrelationId());
    }
    if (lf.getFontData() != null) {
      writeElement(baseXsw, childLevel, "fontData",
          com.mgz.util.UtilCharacterEncoding.bytesToHexString(lf.getFontData()));
    }
    writeIndent(baseXsw, level);
    baseXsw.writeEndElement();
    MnemonicPerformanceMonitor.endWrite();
  ...
  private void writeLcpDirectly(com.mgz.afp.ipds.LCP_LoadCodePage lcp, int level) throws Exception {
    MnemonicPerformanceMonitor.startWriteWithMnemonic("LCP");
    baseXsw.writeStartElement("LCP_LoadCodePage");
    if (currentPageNumber > 0) {
      baseXsw.writeIntAttribute(null, null, "page", currentPageNumber);
    }
    int childLevel = level + 1;
    writeElement(baseXsw, childLevel, "flagByte", (int) lcp.getFlagByte() & 0xFF);
    if (lcp.getCorrelationId() != null) {
      writeElement(baseXsw, childLevel, "correlationId", lcp.getCorrelationId());
    }
    if (lcp.getCodePageData() != null) {
      writeElement(baseXsw, childLevel, "codePageData",
          com.mgz.util.UtilCharacterEncoding.bytesToHexString(lcp.getCodePageData()));
    }
    writeIndent(baseXsw, level);
    baseXsw.writeEndElement();
    MnemonicPerformanceMonitor.endWrite();
  ...
  private void writeLssDirectly(com.mgz.afp.ipds.LSS_LoadSymbolSet lss, int level) throws Exception {
    MnemonicPerformanceMonitor.startWriteWithMnemonic("LSS");
    baseXsw.writeStartElement("LSS_LoadSymbolSet");
    if (currentPageNumber > 0) {
      baseXsw.writeIntAttribute(null, null, "page", currentPageNumber);
    }
    int childLevel = level + 1;
    writeElement(baseXsw, childLevel, "flagByte", (int) lss.getFlagByte() & 0xFF);
    if (lss.getCorrelationId() != null) {
      writeElement(baseXsw, childLevel, "correlationId", lss.getCorrelationId());
    }
    writeElement(baseXsw, childLevel, "flags1", (int) lss.getFlags1() & 0xFF);
    writeElement(baseXsw, childLevel, "scode", lss.getScode());
    writeElement(baseXsw, childLevel, "additionalLength", lss.getAdditionalLength());
    writeElement(baseXsw, childLevel, "flags2", (int) lss.getFlags2() & 0xFF);
    writeElement(baseXsw, childLevel, "xBoxSize", lss.getxBoxSize());
    writeElement(baseXsw, childLevel, "yBoxSize", lss.getyBoxSize());
    writeElement(baseXsw, childLevel, "sectionId", (int) lss.getSectionId() & 0xFF);
    writeElement(baseXsw, childLevel, "ecode", lss.getEcode());
    writeElement(baseXsw, childLevel, "haid", lss.getHaid());
    if (lss.getRestOfHeader() != null) {
      writeElement(baseXsw, childLevel, "restOfHeader",
          com.mgz.util.UtilCharacterEncoding.bytesToHexString(lss.getRestOfHeader()));
    }
    if (lss.getRasterData() != null) {
      writeElement(baseXsw, childLevel, "rasterData",
          com.mgz.util.UtilCharacterEncoding.bytesToHexString(lss.getRasterData()));
    }
    writeIndent(baseXsw, level);
    baseXsw.writeEndElement();
    MnemonicPerformanceMonitor.endWrite();
  ...
  private void writeLeDirectly(com.mgz.afp.ipds.LE_LoadEquivalence le, int level) throws Exception {
    MnemonicPerformanceMonitor.startWriteWithMnemonic("LE");
    baseXsw.writeStartElement("LE_LoadEquivalence");
    if (currentPageNumber > 0) {
      baseXsw.writeIntAttribute(null, null, "page", currentPageNumber);
    }
    int childLevel = level + 1;
    writeElement(baseXsw, childLevel, "flagByte", (int) le.getFlagByte());
    if (le.getCorrelationId() != null) {
      writeElement(baseXsw, childLevel, "correlationId", le.getCorrelationId());
    }
    writeElement(baseXsw, childLevel, "mappingType", le.getMappingType());
    if (le.getRepeatingGroups() != null) {
      writeIndent(baseXsw, childLevel);
      baseXsw.writeStartElement("entries");
      for (IRepeatingGroup irg : le.getRepeatingGroups()) {
        com.mgz.afp.ipds.LE_LoadEquivalence.LE_RepeatingGroup rg =
            (com.mgz.afp.ipds.LE_LoadEquivalence.LE_RepeatingGroup) irg;
        writeIndent(baseXsw, childLevel + 1);
        baseXsw.writeStartElement("entries");
        int rgLevel = childLevel + 2;
        writeElement(baseXsw, rgLevel, "internal", rg.getInternal());
        writeElement(baseXsw, rgLevel, "external", rg.getExternal());
        writeIndent(baseXsw, childLevel + 1);
        baseXsw.writeEndElement();
      }
      writeIndent(baseXsw, childLevel);
  ...
  private void writeWtDirectly(com.mgz.afp.ipds.WT_WriteText wt, int level) throws Exception {
    MnemonicPerformanceMonitor.startWriteWithMnemonic("WT");
    baseXsw.writeStartElement("WT_WriteText");
    if (currentPageNumber > 0) {
      baseXsw.writeIntAttribute(null, null, "page", currentPageNumber);
    }
    int childLevel = level + 1;
    writeElement(baseXsw, childLevel, "flagByte", (int) wt.getFlagByte());
    if (wt.getCorrelationId() != null) {
      writeElement(baseXsw, childLevel, "correlationId", wt.getCorrelationId());
    }
    if (wt.getControlSequences() != null) {
      writeIndent(baseXsw, childLevel);
      baseXsw.writeStartElement("controlSequences");
      for (com.mgz.afp.ptoca.controlSequence.PTOCAControlSequence cs : wt.getControlSequences()) {
        writeControlSequence(cs, childLevel + 1);
      }
      writeIndent(baseXsw, childLevel);
      baseXsw.writeEndElement();
    }
  ...
  private void writeNopIpdsDirectly(com.mgz.afp.ipds.NOP_NoOperation nop, int level) throws Exception {
    MnemonicPerformanceMonitor.startWriteWithMnemonic("NOP");
    baseXsw.writeStartElement("NOP_NoOperation");
    if (currentPageNumber > 0) {
      baseXsw.writeIntAttribute(null, null, "page", currentPageNumber);
    ...
    int childLevel = level + 1;
    writeElement(baseXsw, childLevel, "flagByte", (int) nop.getFlagByte());
    if (nop.getCorrelationId() != null) {
      writeElement(baseXsw, childLevel, "correlationId", nop.getCorrelationId());
    }
    if (nop.getText() != null) {
  ...
  private void writeBpDirectly(com.mgz.afp.ipds.BP_BeginPage bp, int level) throws Exception {
    MnemonicPerformanceMonitor.startWriteWithMnemonic("BP");
    baseXsw.writeStartElement("BP_BeginPage");
    baseXsw.writeIntAttribute(null, null, "page", currentPageNumber);
    int childLevel = level + 1;
    writeElement(baseXsw, childLevel, "flagByte", (int) bp.getFlagByte());
    if (bp.getCorrelationId() != null) {
      writeElement(baseXsw, childLevel, "correlationId", bp.getCorrelationId());
    }
  ...
  private void writeTriplet(XMLStreamWriter2 writer, Triplet triplet, int level) throws Exception {
    ...
    if (triplet instanceof Triplet.Undefined u) {
      ...
      baseXsw.writeIntAttribute(null, null, "codedGraphicCharacterSetGlobalID", fcgcs.codedGraphicCharacterSetGlobalID);
      ...
      baseXsw.writeLongAttribute(null, null, "extendedResourceLocalID", erli.extendedResourceLocalID);
    } else if (triplet instanceof Triplet.ResourceSectionNumber rsn) {
      baseXsw.writeEmptyElement("ResourceSectionNumber");
      baseXsw.writeIntAttribute(null, null, "resourceSectionNumber", (int) rsn.resourceSectionNumber);
    } else if (triplet instanceof Triplet.MediumMapPageNumber mmpn) {
      baseXsw.writeEmptyElement("MediumMapPageNumber");
      baseXsw.writeIntAttribute(null, null, "pageNumber", mmpn.pageNumber);
      ...
      if (oo.nrOfPrecedingObjectsHigh != null) {
        baseXsw.writeLongAttribute(null, null, "nrOfPrecedingObjectsHigh", oo.nrOfPrecedingObjectsHigh);
      }
    } else if (triplet instanceof Triplet.FontHorizontalScaleFactor fhsf) {
      baseXsw.writeEmptyElement("FontHorizontalScaleFactor");
      baseXsw.writeIntAttribute(null, null, "horizontalScaleFactor", (int) fhsf.horizontalScaleFactor);
  ...
  private void writeNameAndTripletsDirectly(com.mgz.afp.base.StructuredFieldBaseName sf, String rootName, int level) throws Exception {
    if (MnemonicPerformanceMonitor.isEnabled()) {
      String mnemonic = MnemonicPerformanceMonitor.extractMnemonicFromString(rootName);
      MnemonicPerformanceMonitor.startWriteWithMnemonic(mnemonic);
    }
    ...
    if (sf instanceof com.mgz.afp.base.IHasTriplets ht) {
      if (ht.getTriplets() != null && !ht.getTriplets().isEmpty()) {
