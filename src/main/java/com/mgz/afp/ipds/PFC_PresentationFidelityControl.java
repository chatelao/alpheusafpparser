...
/**
 * IPDS Presentation Fidelity Control (PFC) Command (X'D634').
 * Refer to IPDS-4-018, IPDS-4-566.
 */
public class PFC_PresentationFidelityControl extends IPDSCommand implements IHasTriplets {

  @AFPField
  private byte fidelityControlFlags;
  ...
  public void decodeAFP(byte[] sfData, int offset, int length, AFPParserConfiguration config)
    ...
    // Byte 0 is reserved
    this.fidelityControlFlags = sfData[offset + consumed + 1];
    // Bytes 2-3 are reserved

    int current = offset + consumed + 4;
    if (actualLength > (current - offset)) {
      this.triplets = TripletParser.parseTriplets(sfData, current, actualLength - (current - offset), config);
    } else {
      this.triplets = null;
    }
  }
  ...
  public void writeAFP(OutputStream os, AFPParserConfiguration config) throws IOException {
    ...
    baos.write(fidelityControlFlags);
  ...
  public byte getFidelityControlFlags() {
    return fidelityControlFlags;
  }

  public void setFidelityControlFlags(byte fidelityControlFlags) {
