 ...
 * <br> The Include Page Segment structured field references a page segment resource object that is
 * to be positioned on the page or overlay. For line-mode or mixed-mode applications only, a value
 * of X'FFFFFF' may be used for either the {@link #xOrigin}, the {@link #yOrigin}, or both.<br> <br>
 * The Include Page Segment structured field references a page segment resource object that is to be
 * presented on the page or overlay presentation space. The IPS specifies a reference point on the
 * including page or overlay coordinate system that may be used to position objects contained in the
 * page segment. A page segment can be referenced at any time during page or overlay state, but not
 * during an object state. The page segment inherits the active environment group definition of the
